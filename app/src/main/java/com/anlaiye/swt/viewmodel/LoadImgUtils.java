...
public class LoadImgUtils {
    ...
    /**
     * 加载头像（头像为空时随机从固定4个种获取一个）
     *
     * @param img
     * @param url
     */
    public static void loadAvatar(ImageView img, String url, String userId) {
        if (TextUtils.isEmpty(url)) {
            NoNullUtils.setImageResource(img, getDefaultAvatar(userId));
        } else if (isService(url)) {
            loadAvatarImageWithThumb(img, getUrl(url, TYPE_AVATAR), userId);
        } else {
            loadAvatarImageWithThumb(img, url, userId);
        }
    }
     ...
     * @param url
     */
    public static void loadAvatarFor7Niu(ImageView img, String url, String userId) {
        if (TextUtils.isEmpty(url)) {
            NoNullUtils.setImageResource(img, getDefaultAvatar(userId));
        } else {
            loadAvatarImageWithThumb(img, url, userId);
    ...
    /**
     * 加载IM头像（头像为空时随机从固定4个种获取一个，显示图90*90
     *
     * @param img
     * @param url
     */
    public static void loadImAvatar(ImageView img, String url, String userId) {
        if (TextUtils.isEmpty(url)) {
            NoNullUtils.setImageResource(img, getDefaultAvatar(userId));
        } else if (isService(url)) {
            loadImageAsBitmap(img, url + STYLE_AVATAR_COMPRESS, userId);
        } else {
            loadImageWithThumb(img, url);
        }
    }
     ...
     * @return
     */
    public static int getDefaultAvatar(String userId) {
        int position;
        if (!TextUtils.isEmpty(userId) && NumberUtils.isInt(userId)) {
            position = (int) (Long.valueOf(userId) % avatarsSize);
        } else {
            position = (int) (Math.random() * avatarsSize);
        }
     ...
     */
    @Deprecated
    public static void loadAvatar(ImageView img, String url) {
        if (null != img) {
            int position = (int) (Math.random() * avatarsSize);
            if (!TextUtils.isEmpty(url)) {
                getIonInstance().build(img)
                        .centerCrop()
                        .placeholder(avatars.get(position))
                        .error(avatars.get(position))
                        .fadeIn(false)
                        .load(url);
            } else {
//                AlyToast.showWarningToast("方法废弃了，赶紧换固定随即头像的方法,否则拉出去接客");
                img.setImageResource(avatars.get(position));
            }
        }
    }
     ...
     * @param url
     */
    public static void loadImageAsBitmap(final ImageView img, String url, final String userId) {
        if (null != img) {
            if (!TextUtils.isEmpty(url)) {
                getIonInstance().build(context)
                    ...
                    public void onCompleted(Exception e, Bitmap result) {
                        if (null != result) {
                            ...
                            img.setImageResource(getDefaultAvatar(userId));
                        }
     ...
     * @param url
     */
    public static void loadImageWithMosaic(final ImageView img, String url) {
        if (null != img) {
            if (!TextUtils.isEmpty(url)) {
                getIonInstance().build(context)
                        .load(url)
                        ...
                        .setCallback(new FutureCallback<Bitmap>() {
                            ...
                            public void onCompleted(Exception e, Bitmap result) {
                                img.setScaleType(ImageView.ScaleType.CENTER_CROP);
                                Bitmap mosaic = MosaicUtil.getGridMosaicBitmapOfNomalImage(
                                        img.getContext(),
                                        result);
                                img.setImageBitmap(mosaic);
                            }
                        });
     ...
     * @param url
     */
    public static void loadImageByType(SimpleDraweeView view, String url, int type) {
        if (TextUtils.isEmpty(url)) {
    ...
    private static void loadAvatarImageWithThumb(final ImageView img, String url, final String userId) {
        if (null != img) {
            if (!TextUtils.isEmpty(url)) {
                getIonInstance().build(context)
                        .load(url)
     ...
     * @param url
     */
    public static void loadImgWithBuy(ImageView img, String url) {
        if (!TextUtils.isEmpty(url)) {
            if (isService(url)) {
                url = getUrl(url, TYPE_B_2);
            }
            getIonInstance().build(img)
                    .placeholder(R.drawable.ic_launcher)
                    .error(R.drawable.ic_launcher)
