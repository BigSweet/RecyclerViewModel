...
public class NoNullUtils {
     ...
     * @param s
     */
    public static void setTextFromNum(TextView t, long s) {
        if (null != t) {
            t.setText(String.valueOf(NumberUtils.getFormatNumber(s)));
        }
    }
     ...
     * @param t
     * @param s
     * @param isBefore      是否放在前面
     * @param imgSpanHeight 当前字体对应sp转换为px（实际上转换时要小2个sp，并且不能设置行间距）
     * @param resId
     */
    public static void setSingleFlagAfterTextView(Context context, TextView t, String s, boolean isBefore, int imgSpanHeight, Integer resId) {
        SpannableString spanStr = null;
        if (null != t) {
            if (null != resId && null != context) {
                Drawable drawable = context.getResources().getDrawable(resId);
                if (null != drawable) {
                    drawable.setBounds(0, 0, getWidth(drawable, imgSpanHeight), imgSpanHeight);//这里设置图片的大小
                    VerticalImageSpan imageSpan = new VerticalImageSpan(drawable);
                    if (isBefore) {
                        spanStr = new SpannableString("  " + s);
                        spanStr.setSpan(imageSpan, 0, 1, Spannable.SPAN_INCLUSIVE_EXCLUSIVE);
                    } else {
                        spanStr = new SpannableString(s = s + "  ");
                        spanStr.setSpan(imageSpan, s.length() - 1, s.length(), Spannable.SPAN_INCLUSIVE_EXCLUSIVE);
                    }
                }
                if (null != spanStr) {
                    t.setText(spanStr);
     ...
     * @param params
     */
    public static void setMultipleFlagAfterTextView(Context context, TextView t, String s, boolean isBefore, int imgSpanHeight, Integer... params) {
        Integer[] resIds = params;
        SpannableString spanStr = null;
        if (null != t) {
            int length;
            if (null != resIds && (length = resIds.length) > 0 && null != context) {
                Integer resId;
                String blank = "";
                ...
                if (isBefore) {
                    spanStr = new SpannableString(blank + s);
                } else {
                    spanStr = new SpannableString(s + blank);
                }

                try {
                    for (int i = 0; i < length; i++) {
                        if (null != (resId = resIds[i])) {
                            if (isBefore) {
                                Drawable drawable = context.getResources().getDrawable(resId);
                                if (null != drawable) {
                                    drawable.setBounds(0, 0, getWidth(drawable, imgSpanHeight), imgSpanHeight);//这里设置图片的大小
                                    VerticalImageSpan imageSpan = new VerticalImageSpan(drawable);
                                    spanStr.setSpan(imageSpan, i * 2, i * 2 + 1, Spannable.SPAN_INCLUSIVE_EXCLUSIVE);
                                }
                            } else {
                                Drawable drawable = context.getResources().getDrawable(resId);
                                if (null != drawable) {
                                    drawable.setBounds(0, 0, getWidth(drawable, imgSpanHeight), imgSpanHeight);//这里设置图片的大小
                                    VerticalImageSpan imageSpan = new VerticalImageSpan(drawable);
                                    spanStr.setSpan(imageSpan, s.length() + 1 + i * 2, s.length() + i * 2 + 2, Spannable.SPAN_INCLUSIVE_EXCLUSIVE);
                                }
                            }
